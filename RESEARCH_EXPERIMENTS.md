...
## 100 轮 `research_runs_v2` 的后续诊断

版本 `909814b7` 的 105 条中，75 条写满记录，30 条被数值保护中止。
`summary.md` 现在只在该条件全部计划种子完成时显示均值；`summary.csv` 保留
失败前的最后轮次、准确率、损失和错误原因。75 条完整记录中还有 29 条曾出现
测试损失超过 100，不能仅凭 `status=complete` 判断训练健康。

该版本在 IPM/ALIE 的发送批次没有良性更新时，用反转服务器历史更新作为回退。
这是另一种攻击，现已改为零更新，并记录累计次数。另增加 `--server-rate`：
服务器以该比例应用聚合更新，默认 1.0 保持原始协议。0.2 在一条曾失败的
IPM 种子上完成了 70 轮，最终测试准确率 0.8313、损失 0.7814；这是探索性
数值复核，需要新种子及无攻击对照，不能与 v2 的方法均值直接合并。

阶段 3 中“20% 标签噪声”也改变了根样本观察到的类别集合，因而与类别覆盖
混杂。尤其根客户端 2、11 不含类别 4、5，不能据此得出标签噪声有益或参考
融合平滑退化的结论。

后续可先运行独立种子 3、4、5 的阶段 1 稳定性检查，计划位于
`research_runs_v3_phase1`；这仍不能检验少数群体误杀，因为当前 MNIST 划分的
主导标签群组规模相同，少数群体指标没有分母。

```powershell
& 'E:\fl-experiment\.venv\Scripts\python.exe' research_suite.py --task ./mnist_dir20 --phases 1 --seeds 3,4,5 --server-rate 0.2 --gpu 0 --output ./research_runs_v3_phase1 --run
```
